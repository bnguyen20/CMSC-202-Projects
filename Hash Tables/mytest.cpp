...
bool testInsertion(HashTable& aTable);
...
int main(){
   ...
   cout << "Message: dump after 5 insertions in a table with 11 buckets:" << endl;
   aTable.dump();
...
bool testInsertion(HashTable& table){
  if(table.insert(Person("Bryan", 5112)))
    return true;
  else
    return false;
}
