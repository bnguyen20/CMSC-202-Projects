...
private:
  ...
  bool recursiveInsert(DNode*& curr, Account newAcct);
  DNode* retrieveHelper(DNode* traverse, int disc);
  void deleteHelper(DNode* nodeToDelete);
  void copyStuff(DNode*& Node, DNode* rhsNode);
  void recursivePrint(DNode* Node) const;
  int findUsers(DNode* Node, int num) const;
  void makeArray(DNode*& Node, DNode** array, int& i);
  DNode* rebalanceArray(DNode** array, int start, int end);
};
