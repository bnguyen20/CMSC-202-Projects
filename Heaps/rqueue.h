...
private:
  ...
  void deleteHelper(Node* nodeToDelete);
  Node* insertHelper(Node* root, Node* nodeToInsert);
  void copyStuff(Node*& node, Node* rhsNode);
  void getStudents(Node* left, RQueue*& rhs);
  void remakeQueue(RQueue*& lhs, Node* rhs);
